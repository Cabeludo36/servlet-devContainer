...
_Vou deixar entre `()` oque eu for colocando caso esteja acompanhando e fazendo_
...
3. Caso pergunte se você que instalar `Maven` ou `Gradle`, não selecione nenhuma, vamos instalar ela no contêiner manualmente, pois SDKMAN pode bugar com `devContaniner` as vezes
### Instalando Dependências Pendentes
Como comentado no paço anterior optamos por não instalar Maven de cara, por conta de alguns bugs que podem ocorrer ente o `SDKMAN` e devContainer

```bash
# Para listar as verções do maven disponiveis
sdk list maven

# Para instalar a verção desejada (em 3.9.10 troque pela versão desejada)
sdk install maven 3.9.10
```

---
## 2. Instalando Extensões
Para uma melhor experiencia de desenvolvimento, é recomendado baixar as seguintes extensões para o VS Code:
### 1. Extension Pack for Java
Ela nos dará uma experiencia de desenvolvedor mais fácil para debugar, compilar e subir novas versões do nosso código para teste
### 2. XML
...
6. De um `Group ID` do projeto (com.example)
7. De um Nome ao projeto
...
- `src/`: é onde editamos o código, temos que ter ela para diferenciarmos das coisas que são buildadas na pasta `target/` que vamos ver para oque serve mais para frente
...
- `meuprojeto/`: que serve para falarmos que queremos abri o VS Code na pasta `meuprojeto/` 
- `-r`: para definir que queremos apenas fazer reload da tela que estamos invés de abir uma nova
...
Para termos uma pasta para colocarmos os nossos servlets e definirmos oque eles vão fazer é necessário criar uma pasta de projeto java, pois no java existe um sistema de arquivos próprio usado para definir pacotes entre outras coisas, por isso não usamos apenas pastas comuns no diretório do projeto e precisamos adicionar como uma pasta de `Source Path` para o Java

Neste caso vou criar a pasta `java/` em `meuprojeto/src/main/` e vou adicionar ela como uma pasta do projeto java clicando com botão direito nela no explorador de arquivos e selecionando a opção `Add Folder to Java Source Path` (Esta opção só aparece caso tenha instalado o `Extension Pack for Java`), caso tenha seguido até aqui corretamente em seu explorador de arquivos ou ao lado de seu terminal terá aparecido uma aba chamada `Java Projects`, se expandirmos ela vamos ver o projeto e uma das pastas vai ser `src/main/java` isso significa que nosso projeto já entendeu que essa é uma pasta para ele ficar de olho quando formos lançar versões/debugarmos/compilarmos nossos arquivos que estão nela

### 2. Definindo um pacote
Depois de criar a pasta no projeto precisamos criar um pacote assim podemos definir de onde buscar os servlets quando o `web.xml` for direcionar usuários para eles, para criar um pacote é bem simples, na nova aba de `Java Projects` clique com o botão direito na pasta que criamos no paço anterior, `new`, selecione `package` e crie um nome ao seu pacote (`com.servletApi`), se você seguiu corretamente verá que no explorador de arquivos terá novas pastas onde o caminho completo forma `src/main/java/com/servletApi` e na aba `Java Projects` abaixo da pasta que criamos terá `com.servletApi`
...
- Clicar com o botão direito na pasta que queremos no explorador de arquivos > `New Java File` e slecionr `Class`;
...
### Liberando o serviço
Este setvlet que criamos irá enviar para o cliente uma pagina HTML, porem nosso serviço web ainda não sabe que ele existe, precisamos declarar sua existencia no arquivo `web.xml`
...
```
- `servlet`: serve para definirmos informações da nossa classe que criamos
- `servlet-name`: é o nome que esse servlet tem no arquivo, como se fosse um nome de uma variável 
- `servlet-mapping`: serve para as informações de mapeamento web desse sevlet
...
#### Subindo um servidor
Para que possamos testar o servlet é necessário termos um servidor, maior parte das fontes java usam o `Tomcat`, ele pode ser instalado na maquina local, mas como estamos em um devContainer porque não queremos instalar coisas localmente podemos usar a extensão `Community Server Connectors` que nos permitira baixar no contêiner e servir nossa aplicação

Na aba `Servers` em seu explorador de arquivos, clique no icone de servidor com um `+` selecione a opção `yes` para baixar um servidor, selecione `Apache Tomcat` > selecione continue, aceite os termos da licença, e ele começará a fazer download do `Tomcat`
Quando o download tiver concluído aparecerá um servidor `Tomcat` com isso podemos simplesmente clicar com o botão direito na pasta de nosso projeto na pasta `target/` (`target/meuprojeto`), e rodar `debug on server` ou `run on server`, neste caso vou debugar > selecionar o servidor que quero rodar a aplicação, selecionar que não quero opções adicionais e dar um nome de projeto para depuração (`meuprojeto`)
Você ira perceber que no canto inferior direito aparecerá uma notificação de que um porte foi aberto do contêiner para a maquina local, se clicar em `Abrir no Browser`, você será redirecionado para a url `http://localhost:8080/` com algumas informações sobre o `Tomcat`, esse é o root do servidor que o `Tomcat` abriu para nós
